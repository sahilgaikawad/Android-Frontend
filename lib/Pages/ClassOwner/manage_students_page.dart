  ...
  Future<void> _fetchStudents() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('http://192.168.1.103:5001/api/student'),
    ...
    if (confirmed == true) {
      try {
        ...
        final response = await http.delete(
          Uri.parse('http://192.168.1.103:5001/api/student/$studentId'),
          headers: {'Authorization': 'Bearer $token'},
        );
        if (mounted) {
          if (response.statusCode == 200) {
...
class _StudentDetailsDialogContentState extends State<_StudentDetailsDialogContent> {
  ...
  Future<void> _fetchStudentDetails() async {
    try {
      ...
      final token = prefs.getString('token');
      final response = await http.get(
        Uri.parse('http://192.168.1.103:5001/api/student/${widget.studentId}'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (mounted) {
        if (response.statusCode == 200) {
